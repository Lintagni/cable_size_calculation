...
function footer(doc: Pdf) {
  ...
  doc.text('Generated by CableCalc — cablecalc.org', 10, 293)
}
...
export async function generateReport(payload: CalcResultPayload, meta?: ReportMeta): Promise<void> {
  ...
  if (payload.type === 'lv') {
    ...
    footer(doc)
    doc.save(`CableCalc-LV-${meta?.circuitId || r.input.description || 'report'}.pdf`)
    return
  }
...
export async function generateBoardReport(
  ...
  doc.setFontSize(11)
  doc.setFont('helvetica', 'bold')
  doc.text('CableCalc  ·  BS7671:2018+A2', 10, 10)
  doc.setTextColor(255, 255, 255)
  ...
  doc.text('Generated by CableCalc — cablecalc.org', 10, 205)

  doc.save(`CableCalc-Board-${meta.boardRef || 'schedule'}.pdf`)
}
